...
from clinic.models import Article, Doctor, Service
...
class Command(BaseCommand):
    help = 'Populate the database with the clinic\'s baseline services, doctors and articles.'

    def add_arguments(self, parser):
        parser.add_argument(
            '--admin-password',
            help='Create a superuser named "admin" with this password if one does not exist.',
        )

    def handle(self, *args, **options):
        created = 0
        for title_en, title_vi, desc_en, desc_vi, icon, category in SERVICES:
            ...
            created += was_created
        self.stdout.write(f'Services: {Service.objects.count()} total ({created} new)')

        created = 0
        for name, specialty, bio_en, bio_vi in DOCTORS:
            _, was_created = Doctor.objects.get_or_create(
                name=name,
                defaults={'specialty': specialty, 'bio_en': bio_en, 'bio_vi': bio_vi},
            )
            created += was_created
        self.stdout.write(f'Doctors: {Doctor.objects.count()} total ({created} new)')

        created = 0
        for title_en, title_vi, content_en, content_vi, slug in ARTICLES:
            ...
            created += was_created
        self.stdout.write(f'Articles: {Article.objects.count()} total ({created} new)')
        ...
        if password:
            ...
            if User.objects.filter(username='admin').exists():
                ...
                User.objects.create_superuser('admin', 'admin@hesclinic.com', password)
